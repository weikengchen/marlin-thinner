...
#[derive(Copy, Clone)]
struct CircuitUnbalanced<F: Field> {
    a: Option<F>,
    b: Option<F>,
    num_constraints: usize,
    num_variables: usize,
...
impl<ConstraintF: Field> ConstraintSynthesizer<ConstraintF> for CircuitUnbalanced<ConstraintF> {
    fn generate_constraints<CS: ConstraintSystem<ConstraintF>>(
        self,
        cs: &mut CS,
    ) -> Result<(), SynthesisError> {
...
mod marlin {
    ...
    fn test_circuit(num_constraints: usize, num_variables: usize) {
        ...
        let universal_srs = MarlinInst::universal_setup(100, 25, 2500, rng).unwrap();
